param(
  [string] $AppPath =
    'test\flt\runtime\build_x64\Debug\crtsys_flt_name_changer_runtime_test_app.exe',
  [string] $VolumeRoot = 'C:\',
  [string] $ServiceName = 'CrtSysFltNameChangerRuntimeTest',
  [string] $DriverName = 'crtsys_flt_name_changer_runtime_test.sys',
...
$resolvedVolume = (Resolve-Path $VolumeRoot).Path
...
& $resolvedApp $resolvedVolume
if ($LASTEXITCODE -ne 0) {
  throw "The minifilter NameChanger runtime app failed with exit code $LASTEXITCODE."
}
...
Write-Host 'NTL minifilter NameChanger runtime and unload verification passed.'
