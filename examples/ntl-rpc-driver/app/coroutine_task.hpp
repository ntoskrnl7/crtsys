...
// The C++ standard defines coroutine machinery but no task return type. This
// small example task starts immediately and stores its result outside the
// coroutine frame. The frame destroys itself after final_suspend so get()
// never races a thread that is still returning from coroutine_handle::resume().
...
public:
    ...
    shared_state()
      ...
      if (!completed)
        throw std::system_error(static_cast<int>(::GetLastError()),
                                std::system_category(),
                                "RPC sample coroutine event creation failed");
    }
  ...
  T get() {
    if (!state_)
      throw std::logic_error("RPC sample coroutine has no result");
    ...
    if (wait != WAIT_OBJECT_0)
      throw std::system_error(static_cast<int>(::GetLastError()),
                              std::system_category(),
                              "RPC sample coroutine wait failed");

    if (state->exception)
      std::rethrow_exception(state->exception);
    if (!state->result)
      throw std::logic_error("RPC sample coroutine produced no result");
