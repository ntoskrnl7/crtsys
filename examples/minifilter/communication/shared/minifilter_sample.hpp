...
NTL_FLT_DECLARE_NOTIFICATION_ID(
    crtsys_minifilter_communication_sample, 0xB10,
    crtsys_minifilter_communication_sample::progress_event, progress)
NTL_FLT_DECLARE_STREAM_ID(crtsys_minifilter_communication_sample, 0xB11,
                          numbers, std::uint32_t, std::uint32_t)

NTL_FLT_RPC_BEGIN_CONTRACT(crtsys_minifilter_communication_sample,
                           L"\\CrtSysMinifilterCommunicationSamplePort", 1,
                           0x3ull)

NTL_FLT_REGISTER_NOTIFICATION(crtsys_minifilter_communication_sample,
                              progress);

NTL_FLT_REGISTER_STREAM(crtsys_minifilter_communication_sample, numbers, value,
                        stream, {
                          auto reply = stream.try_write(value * 2);
                          if (!reply)
                            throw ntl::exception(
                                reply.status(),
                                "Failed to queue stream reply");
                        });

NTL_FLT_ADD_CALLBACK_ID(crtsys_minifilter_communication_sample, 0xA60,
                        std::uint32_t(std::uint32_t), ping,
                        [](std::uint32_t value) noexcept { return value + 1; })

NTL_FLT_ADD_CALLBACK_ID(
    crtsys_minifilter_communication_sample, 0xA62,
    std::uint64_t(std::uint32_t, bool), publish_progress,
    crtsys_minifilter_communication_sample::publish_progress_callback)

NTL_FLT_ADD_CALLBACK_ID(
    crtsys_minifilter_communication_sample, 0xA61,
    ...
    [](const ntl::flt::communication_context &context,
      ...
      crtsys_minifilter_communication_sample::shared_record_ring input_ring;
      crtsys_minifilter_communication_sample::shared_record_ring output_ring;
      if (crtsys_minifilter_communication_sample::shared_record_ring::attach(
              input->data(), input->size(), input_ring) !=
              ntl::ipc::validation_status::success ||
          crtsys_minifilter_communication_sample::shared_record_ring::attach(
      ...
      crtsys_minifilter_communication_sample::shared_record record{};
...
NTL_FLT_RPC_END(crtsys_minifilter_communication_sample)
