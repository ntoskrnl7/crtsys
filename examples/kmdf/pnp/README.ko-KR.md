...
이 예제는 PnP 및 power lifecycle 전반에서 타입이 지정된 NTL 래퍼를 사용하는
...
- idle-policy 구성
- 타입이 지정된 입력/출력 버퍼를 사용하는 PASSIVE-level 순차 큐
...
앱은 device interface를 찾고 타입이 지정된 query IOCTL을 보낸 뒤 prepare-hardware와
