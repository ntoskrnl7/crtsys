...
$inf = Join-Path $PackageDirectory 'crtsys_kmdf_reference.inf'
if (-not (Test-Path -LiteralPath $inf -PathType Leaf)) {
  throw "The built reference INF was not found: $inf"
}

$devcon = Find-DevCon
& $devcon install (Resolve-Path $inf).Path 'Root\CrtSysKmdfReference'
