...
bool query_manual_queue_stats(HANDLE device,
  ...
  return DeviceIoControl(device, kmdf_ntl_sample::manual_stats_ioctl_code,
                         nullptr, 0, &stats, sizeof(stats), &bytes_returned,
                         nullptr) != FALSE &&
...
bool finish_overlapped_ioctl(HANDLE device, OVERLAPPED &overlapped,
  ...
  return GetOverlappedResult(device, &overlapped, &bytes_returned, TRUE) !=
         FALSE;
}
...
int wmain(int argc, wchar_t **argv) {
  ...
  ntl::unique_handle async_device{CreateFileW(
      device_path, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
      FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, nullptr)};
  if (!async_device) {
    std::fwprintf(stderr, L"overlapped CreateFileW failed: %lu\n",
                  GetLastError());
    return 1;
  }

  kmdf_ntl_sample::manual_queue_stats initial_stats{};
  if (!query_manual_queue_stats(device.get(), initial_stats)) {
  ...
  DWORD wait_bytes = 0;
  if (!finish_overlapped_ioctl(async_device.get(), release_overlapped,
                               release_bytes) ||
      !finish_overlapped_ioctl(async_device.get(), wait_overlapped,
                               wait_bytes) ||
  ...
  if (!begin_overlapped_ioctl(
          async_device.get(), kmdf_ntl_sample::manual_wait_ioctl_code,
  ...
  if (!CancelIoEx(async_device.get(), &cancel_overlapped)) {
    std::fwprintf(stderr, L"CancelIoEx failed: %lu\n", GetLastError());
    return 1;
  }

  DWORD cancel_bytes = 0;
  if (finish_overlapped_ioctl(async_device.get(), cancel_overlapped,
                              cancel_bytes) ||
  ...
  if (!query_manual_queue_stats(device.get(), final_stats) ||
