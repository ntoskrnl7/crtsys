...
public:
  tcp_content_filter_state() noexcept
      : executor_(DelayedWorkQueue, "TfWN") {
  ...
  void stream_target(
      ntl::wfp::flow_target<stream_layer, tcp_flow_context> value) noexcept {
    stream_target_ = std::move(value);
  }

  const std::optional<
      ntl::wfp::flow_target<stream_layer, tcp_flow_context>> &
  stream_target() const noexcept {
  ...
  ntl::status defer(const std::shared_ptr<tcp_flow> &flow,
    ...
    try {
      pending = std::make_shared<pending_tcp_message>(
          request_id,
          current_time_100ns() +
              wfp_tcp_content_filter::verdict_timeout_100ns,
          flow, std::move(frame));
    } catch (...) {
      release_pending();
  ...
  std::int32_t submit_verdict(
    ...
    const ntl::status completed = complete(
        *pending,
        verdict == wfp_tcp_content_filter::wire_verdict::permit
            ? tcp_flow_phase::permit_ready
            : tcp_flow_phase::block_ready);
    if (!completed.is_ok()) {
      failed_.fetch_add(1, std::memory_order_relaxed);
      return static_cast<std::int32_t>(
          static_cast<NTSTATUS>(completed));
    }
    if (verdict == wfp_tcp_content_filter::wire_verdict::permit)
  ...
  static void close_flow(
      const std::shared_ptr<tcp_flow> &flow) noexcept {
    if (!flow)
      return;
    spin_lock_guard guard(flow->lock);
...
private:
  ...
  bool insert_pending(
      const std::shared_ptr<pending_tcp_message> &pending) noexcept {
  ...
  static ntl::status
  complete(pending_tcp_message &pending,
...
constexpr auto inspect_tcp =
    +[](const ntl::wfp::stream_event<
            stream_layer, tcp_flow_context> &event) noexcept {
...

      {
        ...
        if (flow->phase == tcp_flow_phase::permit_ready) {
          if (flow->frame_size == 0 ||
              flow->frame_size > data.size()) {
            flow->phase = tcp_flow_phase::closed;
            return ntl::wfp::stream_result::drop_connection();
          }
      ...
      const ntl::status deferred = state->defer(
          flow, std::move(*frame), site, probe.frame_size());
      if (!deferred.is_ok()) {
        tcp_content_filter_state::close_flow(flow);
        return ntl::wfp::stream_result::drop_connection();
      }
...
constexpr auto begin_tcp_flow =
    +[](const ntl::wfp::classify_event<flow_layer> &event) noexcept {
      tcp_content_filter_state *const state = g_state;
      if (!state || !state->accepting() || !state->stream_target())
      ...
      const auto source_port =
          event.value(flow_layer::field::remote_port).uint16();
      const auto destination_port =
          event.value(flow_layer::field::local_port).uint16();
      if (!handle || !protocol || !direction || !source_port ||
          !destination_port || *protocol != IPPROTO_TCP ||
          *direction != FWP_DIRECTION_INBOUND)
        return ntl::wfp::decision::continue_classification;

      try {
        auto flow = std::make_shared<tcp_flow>(
            *source_port, *destination_port);
        std::unique_ptr<tcp_flow_context> context(
            new (std::nothrow) tcp_flow_context(std::move(flow)));
        if (context)
          (void)state->stream_target()->associate(
...
ntl::status ntl::main(ntl::driver &driver, const std::wstring &) {
  ...
  ntl::rpc::server_options options(
      wfp_tcp_content_filter::endpoint_name);
  ...
  auto stream_target =
      callouts->add_stream<tcp_flow_context, inspect_tcp>(
  ...
  if (!stream_target) {
    ...
    return stream_target.status();
  }
  state->stream_target(*stream_target);

  const ntl::status flow_registered =
      callouts->add<begin_tcp_flow>(
          wfp_tcp_content_filter::flow_callout_key);
  if (!flow_registered.is_ok()) {
    state->stop_accepting();
    g_state = nullptr;
    (void)callouts->reset();
    state->wait_for_publishers();
