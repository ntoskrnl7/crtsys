...
using flow_layer = wfp_flow_monitor::flow_layer;
using stream_layer = wfp_flow_monitor::stream_layer;
using query_stats =
...
public:
  ...
  ntl::status wait(std::chrono::milliseconds timeout) noexcept {
    ...
    return KeWaitForSingleObject(
        &handle_.promise().completed, Executive, KernelMode, FALSE,
        &interval);
  }
...
reader_test_task
read_fragmented_message(ntl::net::async_byte_stream &stream,
...
reader_test_task
read_expected_timeout(ntl::net::async_byte_stream &stream) {
...
reader_test_task read_expected_status(
    ntl::net::async_byte_stream &stream, std::size_t count,
    NTSTATUS expected) {
...
ntl::status wait_reader_task(
    reader_test_task &task, ntl::net::async_byte_stream &stream) noexcept {
...
ntl::status validate_coroutine_stream_reader() noexcept {
  auto reader_result =
      ntl::net::async_byte_stream::try_create(16);
  if (!reader_result)
  ...
  constexpr std::array<std::byte, 3> second{
      std::byte{0xfe}, std::byte{0x00}, std::byte{0x03}};
  constexpr std::array<std::byte, 1> third{std::byte{'a'}};
  constexpr std::array<std::byte, 2> fourth{
  ...
  const NTSTATUS body_wait = KeWaitForSingleObject(
      &waiting_for_body, Executive, KernelMode, FALSE,
      &body_wait_timeout);
  ...
  if (!first_status.is_ok() || !second_status.is_ok() ||
      body_wait != STATUS_SUCCESS || !third_status.is_ok() ||
      !fourth_status.is_ok() || !fragmented.is_ok() ||
      !drained.is_ok())
    return STATUS_DATA_ERROR;

  auto timeout_reader_result =
      ntl::net::async_byte_stream::try_create(8);
  ...
  timeout_task.start();
  const ntl::status timed_out =
      wait_reader_task(timeout_task, timeout_reader);
  const ntl::status timeout_drained =
      timeout_reader.cancel_and_wait();
  if (!timed_out.is_ok() || !timeout_drained.is_ok())
    return STATUS_DATA_ERROR;

  auto single_reader_result =
      ntl::net::async_byte_stream::try_create(4);
  if (!single_reader_result)
    return single_reader_result.status();
  auto single_reader = std::move(*single_reader_result);
  auto first_waiter =
      read_expected_status(single_reader, 1, STATUS_CANCELLED);
  ...
  const ntl::status cancelled =
      wait_reader_task(first_waiter, single_reader);
  const ntl::status single_reader_drained =
      single_reader.cancel_and_wait();
  if (!competing.is_ok() || !cancelled.is_ok() ||
      !single_reader_drained.is_ok())
    return STATUS_DATA_ERROR;

  auto eof_reader_result =
      ntl::net::async_byte_stream::try_create(4);
  if (!eof_reader_result)
    return eof_reader_result.status();
  auto eof_reader = std::move(*eof_reader_result);
  auto eof_task =
      read_expected_status(eof_reader, 2, STATUS_END_OF_FILE);
  eof_task.start();
  ...
  if (!partial_status.is_ok() || !eof.is_ok() ||
      !eof_drained.is_ok())
    return STATUS_DATA_ERROR;

  auto limit_reader_result =
      ntl::net::async_byte_stream::try_create(4);
...
struct monitor_state {
  std::optional<ntl::wfp::flow_target<stream_layer, monitor_flow>>
      target;
...
constexpr auto observe_stream =
    +[](const ntl::wfp::stream_event<stream_layer, monitor_flow> &event)
        noexcept {
      monitor_flow *const flow = event.context();
      const auto data = event.data();
      if (flow) {
        flow->state->stream_indications.fetch_add(
            1, std::memory_order_relaxed);
        flow->state->stream_bytes.fetch_add(
            data.size(), std::memory_order_relaxed);
        flow->state->missed_bytes.fetch_add(
            event.missed_bytes(), std::memory_order_relaxed);
      }
      return ntl::wfp::stream_result::permit(data.size());
    };

constexpr auto begin_flow =
    +[](const ntl::wfp::classify_event<flow_layer> &event) noexcept {
      ...
      std::unique_ptr<monitor_flow> context(
          new (std::nothrow) monitor_flow(*state));
      if (!context)
        return ntl::wfp::decision::continue_classification;

      const ntl::status associated =
          state->target->associate(*handle, std::move(context));
      if (associated.is_ok())
...
ntl::status ntl::main(ntl::driver &driver, const std::wstring &) {
#if NTL_HAS_COROUTINE_SUPPORT
  const ntl::status reader_contract =
      validate_coroutine_stream_reader();
  ...
  g_state = state.get();
  auto target =
      callouts->add_stream<monitor_flow, observe_stream>(
  ...
  const ntl::status flow_status =
      callouts->add<begin_flow>(wfp_flow_monitor::flow_callout_key);
