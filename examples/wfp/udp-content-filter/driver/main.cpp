...
struct pending_datagram {
  pending_datagram(ntl::wfp::cloned_packet &&packet_value,
                   ...
                   std::uint64_t endpoint_value,
                   std::uint32_t compartment_value,
                   std::uint32_t remote_address_value) noexcept
      : packet(std::move(packet_value)), deadline(deadline_value),
        endpoint(endpoint_value), compartment(compartment_value),
        remote_address(remote_address_value) {}
...
public:
  explicit udp_content_filter_state(
      ntl::wfp::transport_injector &&injector) noexcept
  ...
  query(ntl::wfp::borrowed_packet packet) const noexcept {
    return injector_.query(packet);
  }
  ...
  ntl::status defer(ntl::wfp::cloned_packet &&packet,
                    ...
                    std::uint32_t remote_address) noexcept {
    if (!accepting() ||
        !ExAcquireRundownProtection(&publish_rundown_))
    ...
    const ntl::status queued = executor_.post(
        [this, request_id, packet = std::move(packet),
         payload = std::move(payload), source_port, destination_port,
         endpoint, compartment, remote_address]() mutable noexcept {
          publish(request_id, std::move(packet), std::move(payload),
                  source_port, destination_port, endpoint, compartment,
                  remote_address);
  ...
  NTSTATUS open_session(std::uint64_t session_id) noexcept {
    ...
    if (active_session_.compare_exchange_strong(
            expected, session_id, std::memory_order_acq_rel))
  ...
  std::int32_t submit_verdict(
      const ntl::rpc::call_context &context, std::uint64_t request_id,
...
private:
  ...
  void publish(std::uint64_t request_id,
               ntl::wfp::cloned_packet &&packet,
               ntl::net::owned_bytes &&payload,
               std::uint16_t source_port,
               std::uint16_t destination_port,
    ...
    std::unique_ptr<pending_datagram> pending(
        new (std::nothrow) pending_datagram(
            std::move(packet),
            current_time_100ns() +
                wfp_udp_content_filter::verdict_timeout_100ns,
            endpoint, compartment, remote_address));
    ...
    for (;;) {
      std::unique_ptr<pending_datagram> expired;
      {
        ...
        for (auto iterator = pending_.begin();
             iterator != pending_.end(); ++iterator) {
  ...
  void drop_all_pending() noexcept {
    std::unordered_map<
        std::uint64_t, std::unique_ptr<pending_datagram>>
        removed;
    {
      fast_mutex_guard guard(pending_lock_);
  ...
  ntl::wfp::transport_injector injector_;
...
constexpr auto inspect_udp =
    +[](const ntl::wfp::classify_event<layer> &event) noexcept {
      ...
      const auto injection_state = state->query(packet);
      ...
      const auto direction =
          event.value(layer::field::direction).uint32();
      const auto protocol =
          event.value(layer::field::protocol).uint8();
      const auto source_port =
          event.value(layer::field::local_port).uint16();
      ...
      if (!direction || !protocol || !source_port || !destination_port ||
          !remote_address || !endpoint || !compartment ||
      ...
      if (payload_size >
        ...
        return ntl::wfp::decision::block_and_absorb;
      auto payload_view =
          bytes.subview(udp_header_size, payload_size);
      if (!payload_view)
        return ntl::wfp::decision::block_and_absorb;
      ...
      // checksum-offload context. IPv4 permits a zero UDP checksum.
      const ntl::status checksum_cleared =
          clone->edit_bytes().write_be16(6, 0);
      if (!checksum_cleared.is_ok())
        return ntl::wfp::decision::block_and_absorb;

      const ntl::status deferred = state->defer(
          std::move(*clone), std::move(*payload), *source_port,
          *destination_port, *endpoint, *compartment, *remote_address);
      (void)deferred;
      return ntl::wfp::decision::block_and_absorb;
    };
...
ntl::status ntl::main(ntl::driver &driver, const std::wstring &) {
  ...
  ntl::rpc::server_options options(
      wfp_udp_content_filter::endpoint_name);
  options.contract_version(wfp_udp_content_filter::contract_version)
      ...
      .max_sessions(4)
      .max_reliable_notifications_per_session(
  ...
  server
      ...
      .on(wfp_udp_content_filter::submit_verdict,
          [state](const ntl::rpc::call_context &context,
            ...
            return state->submit_verdict(
                context, request_id, verdict);
          })
  ...
  const ntl::status registered =
      callouts->add<inspect_udp>(wfp_udp_content_filter::callout_key);
  if (!registered.is_ok()) {
