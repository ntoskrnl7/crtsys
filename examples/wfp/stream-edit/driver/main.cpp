...
struct edit_state {
  ...
  bool self_test_backpressure() noexcept {
    std::size_t reserved = 0;
    while (reserved != wfp_stream_edit::maximum_oob_pending &&
           reserve_pending())
      ++reserved;
    const bool rejected = !reserve_pending();
    while (reserved != 0) {
      release_pending();
      --reserved;
    }
...
private:
  bool reserve_pending() noexcept {
    ...
    while (current < wfp_stream_edit::maximum_oob_pending) {
  ...
  void release_pending() noexcept {
    (void)pending.fetch_sub(1, std::memory_order_acq_rel);
  }
...
constexpr auto edit_stream =
    +[](const ntl::wfp::stream_event<
            stream_layer, edit_flow_context> &event)
        noexcept {
...
constexpr auto begin_edit_flow =
    +[](const ntl::wfp::classify_event<flow_layer> &event) noexcept {
      edit_state *const state = g_state;
...
ntl::status ntl::main(ntl::driver &driver, const std::wstring &) {
  ...
  if (!state->self_test_backpressure())
    return STATUS_INTERNAL_ERROR;
  auto callouts = std::make_shared<ntl::wfp::callout_driver<>>(driver);
  g_state = state.get();

  auto target =
      callouts->add_stream<edit_flow_context, edit_stream>(
