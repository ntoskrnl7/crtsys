...
namespace wfp_stream_edit_app {

/**
 * Top-level sample coroutine owner.
 *
 * The result and completion event live outside the self-destroying coroutine
 * frame, so get() never destroys a frame that an IOCP worker is resuming.
 */
    ...
    shared_state()
      ...
      if (!completed)
        throw std::system_error(
            ...
            "stream-edit coroutine completion event");
    }
