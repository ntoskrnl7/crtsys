...
struct tcp_flow {
  tcp_flow(ADDRESS_FAMILY family_value, std::uint16_t source,
           std::uint16_t destination) noexcept
      : family(family_value), source_port(source),
...
struct pending_tcp_message {
  pending_tcp_message(std::uint64_t request_id_value,
      ...
      : request_id(request_id_value), session_id(session_id_value),
...
public:
  ...
  void record_flow_failure() noexcept {
  ...
  ntl::status defer(const std::shared_ptr<tcp_flow> &flow,
    ...
    if (!accepting() || !flow || !continuation || session_id == 0 ||
        !reserve_pending())
  ...
  void disconnect_session(std::uint64_t session_id) noexcept {
    pending_tcp_slots removed{};
    {
      ...
      for (auto &slot : pending_) {
        if (slot && slot->session_id == session_id)
          removed[destination++] = std::move(slot);
      }
    }
  ...
  std::int32_t submit_verdict(const ntl::rpc::call_context &context,
    ...
    if (verdict != wfp_tcp_content_filter::wire_verdict::permit &&
        ...
        verdict != wfp_tcp_content_filter::wire_verdict::malformed)
    ...
    if (verdict == wfp_tcp_content_filter::wire_verdict::permit)
      ...
      blocked_.fetch_add(1, std::memory_order_relaxed);
      if (verdict == wfp_tcp_content_filter::wire_verdict::malformed)
        malformed_.fetch_add(1, std::memory_order_relaxed);
    }
...
private:
  ...
  void publish(std::uint64_t request_id) noexcept {
    ...
    const std::uint64_t session_id = pending->session_id;
    ...
    request.address_family = static_cast<std::uint16_t>(pending->flow->family);
  ...
  insert_pending(const std::shared_ptr<pending_tcp_message> &pending) noexcept {
    spin_lock_guard guard(pending_lock_);
    if (!pending ||
        active_session_.load(std::memory_order_relaxed) != pending->session_id)
      return false;
    for (auto &slot : pending_) {
      if (!slot) {
        slot = pending;
  ...
  void fail_pending(std::uint64_t request_id) noexcept {
  ...
  void drop_all_pending() noexcept {
    pending_tcp_slots removed{};
    {
      ...
      for (std::size_t index = 0; index != pending_.size(); ++index)
        removed[index] = std::move(pending_[index]);
    }
    cancel_removed(removed);
  }

  void cancel_removed(pending_tcp_slots &removed) noexcept {
    for (auto &pending : removed) {
      if (!pending)
        continue;
      release_pending();
      const ntl::status completed =
...
inspect_tcp(tcp_content_filter_state &owned_state,
            const ntl::wfp::stream_event<StreamLayer, tcp_flow_context>
                &event) noexcept {
  ...
  tcp_content_filter_state *const state = &owned_state;
  if (!state->accepting())
    return ntl::wfp::stream_result::drop_connection();
  if (!flow) {
    state->record_flow_failure();
    return ntl::wfp::stream_result::drop_connection();
  }
  ...
  const ntl::net::framing::u32_be_length_prefix framer{
      wfp_tcp_content_filter::maximum_record_size};
...
ntl::wfp::arbitration_decision
begin_tcp_flow(tcp_content_filter_state &owned_state,
  ...
  try {
    constexpr ADDRESS_FAMILY family =
        std::is_same_v<FlowLayer, flow_layer_v4> ? AF_INET : AF_INET6;
    auto flow = std::make_shared<tcp_flow>(family, *source_port,
                                          *destination_port);
    std::unique_ptr<tcp_flow_context> context(
        new (std::nothrow) tcp_flow_context(std::move(flow)));
    if (context) {
      const ntl::status associated =
          target->associate(*handle, std::move(context));
      if (associated.is_ok())
        return ntl::wfp::arbitration_decision::continue_classification;
    }
  } catch (...) {
...
ntl::status ntl::main(ntl::driver &driver, const std::wstring &) {
  ...
  auto stream_target_v4 = callouts.add_stream<tcp_flow_context>(
      wfp_tcp_content_filter::stream_callout_key_v4, state,
      &inspect_tcp<stream_layer_v4>);
  if (!stream_target_v4) {
  ...
  auto stream_target_v6 = callouts.add_stream<tcp_flow_context>(
      wfp_tcp_content_filter::stream_callout_key_v6, state,
      &inspect_tcp<stream_layer_v6>);
  if (!stream_target_v6) {
    state->stop_accepting();
    (void)callouts.close();
    state->wait_for_publishers();
  ...
  const ntl::status flow_registered_v4 = callouts.add_arbitrating(
      wfp_tcp_content_filter::flow_callout_key_v4, state,
      &begin_tcp_flow<flow_layer_v4, stream_layer_v4>);
  ...
  const ntl::status flow_registered_v6 = callouts.add_arbitrating(
      wfp_tcp_content_filter::flow_callout_key_v6, state,
      &begin_tcp_flow<flow_layer_v6, stream_layer_v6>);
