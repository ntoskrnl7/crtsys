...
std::wstring widen_origin_dns_name(std::string_view value) {
  std::wstring result;
  result.reserve(value.size());
  for (const unsigned char character : value) {
    if (character == 0 || character > 0x7f)
      throw std::invalid_argument(
          "HTTP/3 origin SNI is not an ASCII DNS name");
  ...
  if (*client_identity &&
      !::WinHttpSetOption(
          ...
          const_cast<CERT_CONTEXT *>(
              client_identity->borrowed_certificate()),
          sizeof(CERT_CONTEXT)))
    throw_winhttp(
...
fetch_http_origin_with_transport_fallback_winhttp(
    ...
    ntl::net::inspection::origin_client_identity_provider
        &origin_identities) {
  return fetch_with_safe_transport_fallback(
      method,
      [&] {
        return fetch_origin_winhttp(
            server_name, method, path, headers, body,
      ...
      [](const std::system_error &error) noexcept {
        const std::uint32_t code =
            static_cast<std::uint32_t>(
                error.code().value());
        return (code & 0xffff0000u) == 0x80410000u ||
...
browser_http3_origin_transport::browser_http3_origin_transport(
    ...
    : logger_(std::move(logger)),
      origin_identities_(std::move(origin_identities)), policy_(policy) {}

ntl::result<ntl::net::http3::origin_response>
browser_http3_origin_transport::send(
  ...
  try {
    ...
    auto origin =
        policy_ == http3_origin_policy::require_http3
            ? fetch_http3_origin_winhttp(
                  widen_origin_dns_name(request.server_name),
    ...
    if (origin.headers.empty() &&
      ...
      result.headers.push_back(
          {"content-type", origin.message.content_type});
    result.body = std::move(origin.message.body);
    result.negotiated_protocol =
        std::move(origin.negotiated_protocol);
    if (result.negotiated_protocol != "h3") {
      logger_->record_lifecycle(
          "http3-origin-fallback host=" + request.server_name +
          " upstream=" + result.negotiated_protocol);
    }
