...
void browser_html_logger::record_request(
    std::wstring_view server_name,
  ...
  try {
    ...
    safe_method.reserve((std::min)(method.size(), std::size_t{32}));
    for (const unsigned char character : method.substr(0, 32))
      safe_method.push_back(
          character >= 0x21 && character <= 0x7e
              ? static_cast<char>(character)
              : '?');
    const std::size_t query = path.find('?');
    const std::size_t path_bytes =
        query == std::string_view::npos ? path.size() : query;
    std::lock_guard lock(lock_);
    append_event_locked(
        "request host=" + host +
        " method=" + safe_method +
        " path-bytes=" + std::to_string(path_bytes) +
        " query=" +
        (query == std::string_view::npos ? "absent" : "redacted"));
  } catch (...) {
  }
}
