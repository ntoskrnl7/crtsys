...
이 샘플은 선택한 아웃바운드 IPv4 TCP 스트림을 수정합니다. 인라인 경로는
indication 경계를 가로지르는 `BLOCKME`를 같은 길이의 `REDACT!`로 바꿉니다.
제한된 대역 외 경로는 통과 데이터를 복제하고 `OOBBLOCK`에 대해
가변 길이 대체 데이터를 주입할 수 있습니다.

예제에는 실제 정책과 데이터 경로만 남겨 두었습니다.

- `crtsys_wfp_stream_edit`: 스트림 버퍼링, 주입 완료, 제한된 OOB 작업을
  담당하는 드라이버
- `crtsys_wfp_stream_edit_controller`: 선택한 포트의 ephemeral 정책을
  설치하는 컨트롤러. 소켓이나 코루틴 시험을 만들지 않습니다.
- `crtsys_wfp_stream_edit_acceptance`:
  `test/wfp/runtime/fixtures/kernel/stream-edit`의 픽스처. 클라이언트/서버,
...
기본 허용성 검사는 같은 디렉터리의 컨트롤러를 자식 프로세스로 실행하고,
...
이 모드는 IOCP 읽기/쓰기, 취소, EOF, 분할된 접두사, 합쳐진 프레임 메시지와
드라이버가 실제 사용하는 제한된 OOB 대기 예산을 검증합니다. 해당 시험
코드는 `test`에만 있으며 제품 드라이버 로드 중에는 합성 자체 테스트를 실행하지
않습니다.
