...
`crtsys`는 별도의 STL 포크를 유지하지 않고 Microsoft C++ runtime
생태계(CRT, STL, VCRT, UCRT)를 Windows 커널 드라이버 환경에 통합합니다.
별도로 유지하는 STL 구현을 배포하지 않고 Visual Studio 또는 Build Tools에
설치된 MSVC header와 선택된 runtime source path를 사용하므로, 익숙한 MSVC
개발 경험을 유지하면서 upstream STL과의 차이를 최소화합니다.

Driver code는 익숙한 MSVC C++ header와 STL type을 그대로 사용합니다.
Kernel-specific 동작이 필요한 경로는 include 검색 순서 기반 compatibility
overlay와 kernel runtime layer를 통해 조정합니다. Runtime dependency는 실제
driver-test coverage, 명시적인 lifecycle behavior 및 IRQL contract를 갖춘
kernel-mode substrate 위에 매핑됩니다.

`crtsys`의 목표는 kernel developer가 MSVC toolchain 및 upstream STL과의
정합성을 유지하면서 modern C++ 개발 방식을 사용할 수 있도록 하는 것입니다.
