...
validation. With LDK `0.7.25` and SDK/WDK `10.0.22621.0`, the v143 x64 Debug
and Release drivers also passed VM load, full test execution, and unload
validation, including the runtime-context diagnostics and thread-lifetime
...
| STL owning objects, containers, and callback-heavy utilities | `std::string`, containers, `std::any`, `std::function`, smart pointers, `std::optional`, `std::variant`, `std::complex`, `std::valarray`, `std::filesystem::path`, basic `std::filesystem` file operations, `std::format`, `std::print`, random distributions, `std::regex`, `nlohmann::json`, algorithms that allocate, throw, or call arbitrary user code | `PASSIVE_LEVEL` only | Construction, destruction, comparison, hashing, allocation, deallocation, formatting, file-system I/O, and user callbacks can pull in runtime paths. Debug builds warn when an OS-backed `std::filesystem` path reaches its Win32 boundary above `PASSIVE_LEVEL`; the warning does not make the operation legal. |
| STL synchronization, threading, and async | `std::thread`, `std::jthread`, `std::stop_token`, `std::mutex`, `std::shared_mutex`, `std::condition_variable`, `std::condition_variable_any`, `std::scoped_lock`, `std::lock`, `std::try_lock`, `std::call_once`, `std::future`, `std::shared_future`, `std::promise`, `std::packaged_task` | `PASSIVE_LEVEL` only | The driver must request long-running workers to stop and join them before runtime teardown. Semantic coverage verifies `jthread` destructor stop/join, explicit `request_stop()`, and exactly-once stop callbacks; the driver fixture also keeps a worker active until unload and stops/joins it before releasing runtime-owned state. crtsys then blocks new LDK-created threads and waits for existing threads before destroying CRT state. |
...
| NTL entry, driver, device, and RPC server helpers | `ntl::main`, `ntl::driver`, `ntl::device`, `ntl::rpc::server` | `PASSIVE_LEVEL` only | Intended for initialization, teardown, device setup, and IOCTL/RPC control paths. RPC preserves the original caller security context and supports pre-deserialization method authorization. |
...
| NTL pool ownership and allocator | `ntl::pool_ptr`, `ntl::pool_buffer`, `ntl::pool_allocator`, `ntl::nonpaged_pool_allocator`, `ntl::paged_pool_allocator`, `ntl::pmr::pool_resource` | Raw nonpaged pool `<= DISPATCH_LEVEL`; raw paged pool `<= APC_LEVEL`; generic STL/PMR use is `PASSIVE_LEVEL` | Exposes native and allocator policy checks and emits Debug warnings when STL/PMR allocation hooks run above `PASSIVE_LEVEL`. The allocator cannot observe capacity-preserving operations, and nonpaged storage alone does not make arbitrary STL work DPC-safe. |
...
| NTL process-mapped I/O and minifilter swapped buffers | `ntl::ipc::try_map_io_buffers`, `ntl::ipc::try_map_completed_io_buffers`, typed `ntl::flt::try_map_io_buffers`, typed `ntl::flt::try_swap_io_buffers`, pending/deferred helpers | Mapping, target-process unmapping, swapping setup, service waits, and copy-back are `PASSIVE_LEVEL`; transferred cleanup-only swapped contexts, cancel-queue callbacks, and deferred-work submission are separately DISPATCH-safe | Automatically derives logical input/output from IRP or IOPB metadata, infers the only valid swap direction for single-direction operations, requires an explicit selector only for dual-capable operations, isolates partial-page/kernel backing by default, and keeps mapping, swapping, transport, and pending ownership separate. See [I/O buffer mapping](./ntl/io-buffer-mapping.md). |
...
| NTL Filter Manager work item | `ntl::flt::queue_instance_work_item`, `ntl::flt::queue_filter_work_item` | Submission `<= DISPATCH_LEVEL`; callback runs at `PASSIVE_LEVEL` | Queues through `FltQueueGenericWorkItem`, rejects work once Filter Manager deletion starts, and retains filter rundown until the callback returns. Use it instead of a raw executive work item for minifilter-owned detached work. |
| NTL kernel coroutine context | C++20 `ntl::resume_on_passive` | Called through `DISPATCH_LEVEL`; a successfully deferred continuation resumes at `PASSIVE_LEVEL` | Controls only an explicit await point, not arbitrary `coroutine_handle::resume()`. Queue failure is returned as `ntl::status` in the original context, so callers must check it before passive-only work. |
...
| NTL RPC client | `ntl::rpc::client` | User mode, not kernel IRQL | Shared macros generate both sides over typed method descriptors. Bounded replies, malformed/trailing payload rejection, and x86-client/x64-driver schemas are covered. |
