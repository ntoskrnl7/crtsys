...
| 값 | 소유자 | 유효성 및 이전 규칙 |
| --- | --- | --- |
| `driver`, `device`, `file`, `io_queue`, `io_target`, `memory`, `timer`, `work_item`, `interrupt`, `dma_*`, `wmi_*` | WDF | 비소유 래퍼입니다. 네이티브 WDF 객체의 수명 너머로 보관하지 마세요. 오래 사는 자식에는 부모를 명시적으로 지정하세요. |
| 큐가 전달한 `request` | 하나의 종료 동작 전까지 드라이버 콜백 | 완료, 전달, 재큐잉, 전송할 수 있는 이동 전용 권한입니다. 성공적으로 이전하면 원본 요청 객체는 무효가 됩니다. |
...
- 사용자/커널 계약에는 고정 폭 필드를 쓰고 모든 32/64비트 공용 레이아웃을
