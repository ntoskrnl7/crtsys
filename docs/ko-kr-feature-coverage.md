...
local build/package 및 VM 검증을 기록합니다. LDK `0.7.25`와 SDK/WDK
`10.0.22621.0`으로 빌드한 v143 x64 Debug/Release driver도 VM에서 load,
전체 test 실행, unload를 통과했으며 아래 runtime-context 진단과 thread
lifetime coverage가 이 검증에 포함됩니다.
...
| STL owning object, container, callback-heavy utility | `std::string`, container, `std::any`, `std::function`, smart pointer, `std::optional`, `std::variant`, `std::complex`, `std::valarray`, `std::filesystem::path`, 기본 `std::filesystem` file operation, `std::format`, `std::print`, random distribution, `std::regex`, `nlohmann::json`, allocation/throw/arbitrary user code 호출 가능성이 있는 algorithm | `PASSIVE_LEVEL` only | 생성, 소멸, 비교, hashing, allocation, deallocation, formatting, file-system I/O, user callback이 runtime path를 끌어올 수 있습니다. Debug build는 OS-backed `std::filesystem` 경로가 `PASSIVE_LEVEL`보다 높은 곳에서 Win32 경계에 도달하면 경고하며, 그 경고가 해당 호출을 허용한다는 뜻은 아닙니다. |
| STL synchronization, threading, async | `std::thread`, `std::jthread`, `std::stop_token`, `std::mutex`, `std::shared_mutex`, `std::condition_variable`, `std::condition_variable_any`, `std::scoped_lock`, `std::lock`, `std::try_lock`, `std::call_once`, `std::future`, `std::shared_future`, `std::promise`, `std::packaged_task` | `PASSIVE_LEVEL` only | 드라이버는 runtime teardown 전에 장기 실행 worker에 종료를 요청하고 join해야 합니다. semantic test는 `jthread` destructor의 stop/join, 명시적 `request_stop()`, stop callback의 정확히 한 번 실행을 검증하며, driver fixture도 unload까지 worker를 실행한 뒤 runtime-owned state를 해제하기 전에 stop/join합니다. 그 다음 crtsys가 LDK 기반 새 스레드 생성을 막고 기존 스레드 종료를 기다린 뒤 CRT 상태를 정리합니다. |
...
| NTL timer and DPC wrapper | `ntl::timer`, `ntl::kdpc`, `relative_due_time_ms` | timer setup/cancel은 WDK timer 규칙을 따름; timer wait는 NTL usage에서 `PASSIVE_LEVEL`; DPC callback은 `DISPATCH_LEVEL` | one-shot timer, periodic timer, direct DPC queueing, timer DPC callback을 감쌉니다. DPC callback은 resident, 짧고, nonblocking이어야 하며 arbitrary STL/CRT work를 하지 않아야 합니다. |
