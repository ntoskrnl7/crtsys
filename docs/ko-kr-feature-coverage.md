...
| NTL handle/object ownership | `ntl::unique_kernel_handle`, `ntl::unique_object`, `try_reference_object_by_handle` | 정확한 WDK primitive가 더 넓은 contract를 문서화하지 않는 한 `PASSIVE_LEVEL` | `ZwClose` handle ownership과 `ObDereferenceObject` reference ownership을 분리합니다. |
| NTL registry helper | `ntl::registry_key`, `ntl::registry_value`, `try_open_driver_parameters`, `open_driver_parameters` | `PASSIVE_LEVEL` only | driver configuration path에서 Zw registry key handle을 RAII로 관리하고 typed `REG_*` value query/set helper를 제공합니다. |
| NTL pool ownership and allocator | `ntl::pool_ptr`, `ntl::pool_buffer`, `ntl::pool_allocator`, `ntl::nonpaged_pool_allocator`, `ntl::paged_pool_allocator`, `ntl::pmr::pool_resource` | raw nonpaged pool은 WDK pool 규칙을 따름; object construction/destruction과 STL/PMR 사용은 별도 감사 전에는 `PASSIVE_LEVEL` | WDK pool kind/option을 RAII ownership helper, STL-compatible allocator, PMR resource type으로 노출합니다. |
...
| NTL work item wrapper | `ntl::work_item`, `ntl::passive_work_item` | `queue()`는 `<= DISPATCH_LEVEL`; `wait()`와 worker callback ownership은 `PASSIVE_LEVEL` | resident work를 `PASSIVE_LEVEL` system worker thread로 넘깁니다. |
...
- [x] `ntl::unique_kernel_handle` / `ntl::unique_object`
  - [x] kernel handle close, move, release, adopt path
