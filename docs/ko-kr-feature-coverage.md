...
| NTL registry helper | `ntl::registry_key`, `ntl::registry_value`, `ntl::driver_config`, `try_open_driver_parameters`, `open_driver_parameters` | `PASSIVE_LEVEL` only | driver configuration path에서 Zw registry key handle을 RAII로 관리하고 typed `REG_*` value query/set/default helper를 제공합니다. |
