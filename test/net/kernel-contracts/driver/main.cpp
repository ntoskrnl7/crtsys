...
void run_msquic_facade_close_race(void *context) noexcept {
  ...
  auto initially_retained = probe.configuration->make_connection_context();
...
ntl::net::kernel::started_task capture_kernel_tls_close(
    ntl::net::kernel::task<ntl::status> operation,
    std::atomic<NTSTATUS> &result) {
  const ntl::status closed = co_await std::move(operation);
  result.store(static_cast<NTSTATUS>(closed), std::memory_order_release);
}
...
wsk_tls_round_trip(
  ...
  reply.field_count = 6;
  // Start both closes before the peer sends close_notify. The second call
  // must join the first operation without cancelling its authenticated peer
  // alert read or starting another transport close sequence.
  std::atomic<NTSTATUS> first_close{STATUS_PENDING};
  std::atomic<NTSTATUS> second_close{STATUS_PENDING};
  auto first = capture_kernel_tls_close(stream.close(), first_close);
  auto second = capture_kernel_tls_close(stream.close(), second_close);
  co_await std::move(first);
  co_await std::move(second);
  const ntl::status closed{
      first_close.load(std::memory_order_acquire)};
  if (!closed.is_ok() ||
      second_close.load(std::memory_order_acquire) !=
