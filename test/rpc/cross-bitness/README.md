...
- truncated input, an oversized container count, and an unknown callback ID,
  followed by a valid request to prove the endpoint remains usable.
...
```text
RPC cross-bitness PASS: client=x86 server=x64 boundary=1 empty=1 large_bytes=131072 large_numbers=32768 bounded_response=1 malformed=3
```
