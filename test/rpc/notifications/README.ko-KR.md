...
이 fixture는 보류 중인 `METHOD_BUFFERED` IOCTL 및 커널 `IO_CSQ` cancel-safe queue를 통한 타입이 지정된 커널-사용자 모드 알림을 검증합니다.

검사 범위:

- x64 driver와 x86/x64 app 전반의 고정 폭 알림 선택
- `std::string`, `std::vector<std::uint32_t>` payload 직렬화
- 동기 및 overlapped 타입이 지정된 수신
...
- 타입이 지정된 session state와 외부 persistence hook 호출
