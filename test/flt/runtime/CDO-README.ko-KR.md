...
미니필터는 `driver.start()` 전에 이름 있는 `ntl::device<cdo_extension>`를 queue에 넣습니다. NTL은 filter 등록과 filtering 시작 사이에 이를 만들고, 형식화된 create/cleanup/close/device-control handler를 구성한 후 `\\DosDevices\\CrtSysFltCdoRuntime`을 공개합니다.

VM app은 다음을 검증합니다.

- `\\.\CrtSysFltCdoRuntime`을 사용자 모드에서 열 수 있음
- 두 번째 동시 create가 driver의 single-open 실패를 받음
- 공유 `ntl::ioctl_from_contract` ping이 형식화된 입력과 출력을 검증함
