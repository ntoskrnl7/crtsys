...
이 픽스처는 공개 소프트웨어 전용 KMDF 예제를 하나의 관찰 가능한 VM 게이트로
실행합니다. 같은 드라이버의 두 번째 사본을 유지하는 대신 읽기 쉬운 기존 예제를
의도적으로 재사용합니다.
...
- 버전이 있는 참조 ABI, 파일별 세션, 지연 완료, 취소
- 동적 PDO 플러그, 쿼리, missing 처리, 재플러그, eject
- 상위 필터 전달과 형식화된 완료
...
자식 함수 INF 패키지가 모두 있어야 합니다. 교차 비트 수 검증이 필요하면, 두 번째
루트 아래에 같은 디렉터리 이름으로 x86 애플리케이션을 준비하세요.
...
사용하세요. 수용 래퍼도 같은 목적으로 `-SkipBuild`를 받습니다. 빈
`RestoreDriverFileName`은 이전 구성을 추측하는 대신 Verifier를 재설정합니다.

## 수용 기준

완전한 호스트 수용 실행은 다음 조건을 모두 충족해야 합니다.
