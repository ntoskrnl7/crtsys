...
// Deterministic callback fragmentation and codec replay are acceptance-only.
#include <cstddef>
#include <cstdint>
#include <iostream>
#include <span>
...
int run_replay_contract() {
  const std::array<ntl::net::http3::header_field, 2> repeated_block_headers{{
      {"x-ntl-block", "1"},
      {"x-ntl-block", "0"},
  }};
  if (!crtsys::examples::wfp::http3_inspection::block_requested(
          std::span<const ntl::net::http3::header_field>(
              repeated_block_headers))) {
    std::cerr << "HTTP/3 repeated block header bypassed policy\n";
    return 1;
  }

  const std::array<ntl::net::http3::header_field, 3>
      ordinary_block_headers{{
          {":method", "GET"},
          {":path", "/blocked"},
          {"x-ntl-block", "1"},
      }};
  if (!crtsys::examples::wfp::http3_inspection::ordinary_request_blocked(
          std::span<const ntl::net::http3::header_field>(
              ordinary_block_headers))) {
    std::cerr << "HTTP/3 ordinary request did not use the shared policy\n";
    return 2;
  }
  ntl::net::http::inspection_policy ordinary_policy;
  crtsys::examples::wfp::http3_inspection::configure_ordinary_policy(
      ordinary_policy);
  ntl::net::http::request_message ordinary_request;
  ordinary_request.wire_protocol = ntl::net::http::protocol::http3;
  ordinary_request.method = "GET";
  ordinary_request.path = "/blocked";
  ordinary_request.headers.append("x-ntl-block", "1");
  ntl::net::http::inspection_session_metadata inspection_session;
  const auto ordinary_context = ntl::net::http::inspection_context_view::for_request(
      ntl::net::http::protocol::http3, 4, 4,
      ntl::net::http::inspection_stage::headers, inspection_session,
      ordinary_request);
  if (ordinary_policy.decisions_ref().evaluate(ordinary_context) !=
      ntl::net::inspection::verdict::block) {
    std::cerr << "HTTP/3 inspection_policy did not block the shared rule\n";
    return 3;
  }
      ...
      .bytes = std::vector<std::byte>(
          reinterpret_cast<const std::byte *>(
              crtsys::examples::wfp::http3_inspection::
                  webtransport_fixture_input.data()),
          reinterpret_cast<const std::byte *>(
              crtsys::examples::wfp::http3_inspection::
                  webtransport_fixture_input.data() +
              crtsys::examples::wfp::http3_inspection::
                  webtransport_fixture_input.size()))};
  const auto webtransport_outcome =
      webtransport_policy.apply(webtransport_payload);
  const std::string_view webtransport_text(
      reinterpret_cast<const char *>(webtransport_payload.bytes.data()),
      webtransport_payload.bytes.size());
  if (!webtransport_outcome.modified ||
      webtransport_text !=
          crtsys::examples::wfp::http3_inspection::
              webtransport_fixture_output) {
    std::cerr << "HTTP/3 WebTransport did not use the shared transform\n";
    return 4;
  }
  ...
  const auto inspected = backend.run_borrowed(adapter);
