    ...
    if (server_) {
        ...
        return STATUS_DATA_ERROR;
      {
        ...
        const ntl::status accepted =
            session_->accept_client_response(stream_id, 200);
        if (!accepted.is_ok())
          return accepted;
        headers_seen_ = true;
...
void verify_owning_runtime_lifetime() {
  using ntl_runtime =
      ntl::net::http3::msquic_backend::runtime;
  using ntl_configuration =
      ntl::net::http3::msquic_backend::configuration;

  ntl_runtime owner;
  require(
      owner.open("crtsys-ntl-msquic-lifetime-contract").is_ok(),
      "owning MsQuic runtime open failed");

  constexpr std::array<std::string_view, 1> protocols{"h3"};

  ntl_configuration shared_configuration;
  require(
      shared_configuration.open(owner, protocols).is_ok(),
      "shared MsQuic configuration open failed");
  ntl_configuration configuration_alias = shared_configuration;
  auto alias_retained_native_state =
      configuration_alias.make_connection_context();
  require(
      static_cast<bool>(alias_retained_native_state),
      "MsQuic configuration alias did not retain native state");

  configuration_alias.close();
  configuration_alias.close();
  require(
      !configuration_alias && !shared_configuration,
      "explicit MsQuic configuration close did not reject work through aliases");
  require(
      static_cast<bool>(alias_retained_native_state),
      "configuration close invalidated an existing MsQuic child");
  alias_retained_native_state = {};

  ntl_configuration raced_configuration;
  require(
      raced_configuration.open(owner, protocols).is_ok(),
      "raced MsQuic configuration open failed");
  std::atomic<bool> close_complete{false};
  std::atomic<bool> accepted_after_close{false};
  std::thread user([&] {
    while (!close_complete.load(std::memory_order_acquire))
      (void)raced_configuration.make_connection_context();
    for (unsigned attempt = 0; attempt != 1024; ++attempt) {
      if (raced_configuration.make_connection_context()) {
        accepted_after_close.store(true, std::memory_order_release);
        break;
      }
    }
  });
  std::thread closer([&] {
    // Deliberately race close/use on the same public facade, not merely on
    // two aliases that happen to share native state.
    raced_configuration.close();
    raced_configuration.close();
  ...
  user.join();
  require(
      !accepted_after_close.load(std::memory_order_acquire) &&
          !raced_configuration,
      "MsQuic configuration close/use race accepted new work after close");

  ntl_configuration child;
  require(
      child.open(owner, protocols).is_ok(),
      "owning MsQuic configuration open failed");
  auto retained_native_state = child.make_connection_context();
  require(
      static_cast<bool>(retained_native_state),
      "owning MsQuic configuration did not retain native state");

  ntl::net::http3::msquic_backend::server listener;
  require(
      listener.open(
          owner,
          [child](const auto &) -> ntl::result<ntl_configuration> {
            return ntl::ok(child);
          },
          [](std::shared_ptr<ntl::net::quic::transport_backend>,
            ...
            return ntl::unexpected(STATUS_CONNECTION_REFUSED);
          })
          .is_ok(),
      "owning MsQuic listener open failed");
  QUIC_ADDR listener_address{};
  QuicAddrSetFamily(&listener_address, QUIC_ADDRESS_FAMILY_INET);
  QuicAddrSetToLoopback(&listener_address);
  QuicAddrSetPort(&listener_address, 0);
  require(listener.start(protocols, &listener_address).is_ok(),
          "owning MsQuic listener start failed");
  std::atomic<bool> listener_close_complete{false};
  std::atomic<unsigned> listener_observations{0};
  std::thread listener_user([&] {
    while (!listener_close_complete.load(std::memory_order_acquire)) {
      (void)listener.local_address();
      (void)listener.statistics();
      listener_observations.fetch_add(1, std::memory_order_release);
    }
    (void)listener.local_address();
    (void)listener.statistics();
  });
  while (listener_observations.load(std::memory_order_acquire) == 0)
    std::this_thread::yield();
  std::thread listener_closer([&] {
    listener.close();
    listener.close();
    listener_close_complete.store(true, std::memory_order_release);
  });
  listener_closer.join();
  listener_user.join();
  require(listener_observations.load(std::memory_order_acquire) != 0,
          "same-facade MsQuic listener close/use race did not execute");

  std::atomic<bool> runtime_close_complete{false};
  std::atomic<bool> runtime_accepted_after_close{false};
  std::thread runtime_user([&] {
    while (!runtime_close_complete.load(std::memory_order_acquire)) {
      (void)static_cast<bool>(owner);
      (void)owner.borrowed_native_api();
    }
    for (unsigned attempt = 0; attempt != 1024; ++attempt) {
      if (owner) {
        runtime_accepted_after_close.store(true, std::memory_order_release);
        break;
      }
    }
  });
  std::thread runtime_closer([&] {
    owner.close();
    owner.close();
    runtime_close_complete.store(true, std::memory_order_release);
  });
  runtime_closer.join();
  runtime_user.join();
  require(!owner, "closed MsQuic runtime accepted new work");
  require(
      !runtime_accepted_after_close.load(std::memory_order_acquire),
      "same-facade MsQuic runtime close/use race accepted new work");
  require(
      !child,
      "configuration remained available for new work after runtime close");

  child.close();
  child.close();
  require(
      static_cast<bool>(retained_native_state),
      "owner-first close invalidated retained MsQuic child state");

  ntl_configuration rejected;
  require(
      !rejected.open(owner, protocols).is_ok(),
      "closed MsQuic runtime created a new configuration");
  retained_native_state = {};
}

int run_test() {
  verify_owning_runtime_lifetime();
  ...
  auto client_result = backend_connection::try_connect_borrowed(
      runtime.api(), runtime.registration(),
      runtime.client_configuration(),
  ...
  server_sink.attach(listener_state.accepted);
  ...
  client->stop();
  server->stop();
  require(client->drain().is_ok(),
          "client connection shutdown timed out");
  require(server->drain().is_ok(),
