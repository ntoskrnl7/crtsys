...
std::string grpc_payload_text(std::span<const std::byte> wire) {
  ...
  if (!header || header->compressed || wire.size() != 5 + header->payload_size)
    throw std::runtime_error("controlled HTTP/3 gRPC message is invalid");
  return {reinterpret_cast<const char *>(wire.data() + 5),
          header->payload_size};
}
    ...
    try {
      if (request.server_name !=
        ...
        return ntl::ok(make_text_response(
            ...
            bytes_of(
                ...
                "controlled-h3.test.\n")));
      const bool grpc = request.method == "POST" && request.path == "/grpc";
      if (request.method != "GET" && !grpc)
        return ntl::ok(make_text_response(
      ...
      if (grpc) {
        const auto content_type = std::find_if(
            ...
            [](const auto &field) {
              return field.name == "content-type";
            });
        if (content_type == request.headers.end() ||
          ...
          return ntl::ok(make_text_response(
              ...
              bytes_of("gRPC request was not transformed\n")));
        return ntl::ok(make_text_response(
            200, "application/grpc", request.body));
      }
...
void exercise_controlled_grpc(
  ...
  ntl::net::http3::msh3_client::private_ca_client client(
      ...
      {.maximum_request_headers = 128,
       ...
       .maximum_response_body_bytes = controlled_upstream_body_limit,
       .connect_timeout = std::chrono::seconds(10),
       .response_timeout = std::chrono::seconds(15),
       .shutdown_timeout = std::chrono::seconds(5)});
  const auto fixture = std::as_bytes(std::span(
      crtsys::wfp_browser_http_policy::grpc_transform_fixture));
  const auto encoded = ntl::net::grpc::encode_message(fixture, false, 4096);
  if (!encoded)
    throw std::runtime_error("cannot encode controlled HTTP/3 gRPC fixture");
  ntl::net::http3::msh3_client::request request;
  request.server_name = narrow_dns_name(controlled_host);
  request.port = 443;
  request.peer =
      ntl::net::http3::msh3_client::peer_endpoint::ipv4_loopback(proxy_port);
  request.method = "POST";
  request.path = "/grpc";
  request.headers.push_back({"content-type", "application/grpc"});
  ...
  if (!response || response->status != 200 ||
    ...
    throw std::runtime_error(
        "controlled HTTP/3 gRPC request/response transform failed");
  std::cout
      << "NTL controlled HTTP/3 gRPC wire PASS: "
         "request=transformed response=transformed protocol=h3\n";
}
...
int run_controlled_http3_end_to_end(
  ...
  auto proxy_logger = std::make_shared<browser_html_logger>(
      log_directory / L"proxy");
  auto origin_logger = std::make_shared<browser_html_logger>(
  ...
  origin_authority->export_public_certificate(
      origin_ca_path);
  auto origin_issuer = std::make_shared<
  ...
  auto origin_application =
      std::make_shared<controlled_origin_application>();
  auto origin_policy =
  ...
  browser_http3_service origin_service(
      ...
      origin_application, origin_policy);

  auto origin_transport =
      std::make_shared<controlled_msh3_origin_transport>(
  ...
  inspection_authority->export_public_certificate(
      inspection_ca_path);
  auto inspection_issuer = std::make_shared<
      ntl::net::windows_tls_certificate_issuer>(
  ...
  exercise_controlled_grpc(
      inspection_authority->get(), proxy_port);
