...
#define BSWAP32(value)                                                                                                 \
    (uint32_t)(((uint32_t)(value) >> 24) | (((uint32_t)(value) >> 8) & 0x0000FF00) |                                   \
               (((uint32_t)(value) << 8) & 0x00FF0000) | ((uint32_t)(value) << 24))

// for constant value
#define MASM_BSWAP32(value)                                                                                            \
    ((((value)shr 24) and 0x000000FF) or (((value)shr 8) and 0x0000FF00) or (((value)shl 8) and 0x00FF0000) or         \
     (((value)shl 24) and 0xFF000000))

// for constant value
#define MASM_BSF32(x)                                                                                                  \
    (-1 + ((x) and 1) + ((((x)shr 1) and 1) and (((x) and 0x00000001) eq 0)) * 2 +                                     \
...
#define XCR_AVX512F (XSTATE_OPMASK | XSTATE_ZMM | XSTATE_HI_ZMM)
    ...
    intel_outside = (cpuInfo.ebx ^ BSWAP32('Genu')) | (cpuInfo.edx ^ BSWAP32('ineI')) | (cpuInfo.ecx ^ BSWAP32('ntel'));
...
ISA_AVAILABLE_AVX2:
    ...
    __isa_enabled =
        __ISA_ENABLED_X86 | __ISA_ENABLED_SSE2 | __ISA_ENABLED_SSE42 | __ISA_ENABLED_AVX | __ISA_ENABLED_AVX2;
    return 0;

ISA_AVAILABLE_AVX512:
    __isa_available = __ISA_AVAILABLE_AVX512;
    __isa_enabled = __ISA_ENABLED_X86 | __ISA_ENABLED_SSE2 | __ISA_ENABLED_SSE42 | __ISA_ENABLED_AVX |
                    __ISA_ENABLED_AVX2 | __ISA_ENABLED_AVX512;
    return 0;
