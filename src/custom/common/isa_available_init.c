...
#define BSWAP32(value)                                                         \
  (uint32_t)(                                                                  \
      ((uint32_t)(value) >> 24) | (((uint32_t)(value) >> 8) & 0x0000FF00) |    \
      (((uint32_t)(value) << 8) & 0x00FF0000) | ((uint32_t)(value) << 24))

// for constant value
#define MASM_BSWAP32(value)                                                    \
  ((((value)shr 24) and 0x000000FF) or (((value)shr 8) and 0x0000FF00) or      \
   (((value)shl 8) and 0x00FF0000) or (((value)shl 24) and 0xFF000000))
...
#define XCR_AVX512F (XSTATE_OPMASK | XSTATE_ZMM | XSTATE_HI_ZMM)
  ...
  intel_outside = (cpuInfo.ebx ^ BSWAP32('Genu')) |
  ...
  if (cpuid_0_eax >= 7) {
    ...
    __favor |= ((cpuid_7_ebx = cpuInfo.ebx) & CX_ERMS) >>
               (BSF32(CX_ERMS) - __FAVOR_ENFSTRG);
  }
  if (cpuid_1_ecx & CF_SSE42)
    if ((cpuid_1_ecx & C1_AVX) == C1_AVX &&
        ((xgetbv_eax = (uint32_t)_xgetbv(0)) & XCR_AVX) == XCR_AVX)
      if (cpuid_7_ebx & CX_AVX2)
        if ((cpuid_7_ebx & C7_AVX512) == C7_AVX512 &&
            (xgetbv_eax & XCR_AVX512F) == XCR_AVX512F)
...
#define C7_AVX512                                                              \
  (CX_AVX512F or CX_AVX512DQ or CX_AVX512CD or CX_AVX512BW or CX_AVX512VL)
